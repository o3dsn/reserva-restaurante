...
public class ReservaJPARepositoryTest {
    ...
    class Criar {
        ...
        void devePermitirCriarReserva() {
            ...
            when(reservaJPARepositoryMock.save(any(ReservaJPAEntity.class)))
                    .thenReturn(reserva);
    ...
    class Buscar {
        ...
        void devePermitirBuscarTodasReservas() {
            ...
            when(reservaJPARepository.findAll(any(Pageable.class))).thenReturn(new PageImpl<>(listOfReserva));
        ...
        void devePermitirBuscarReservaPorId() {
            ...
            when(reservaJPARepository.findById(String.valueOf(any(String.class)))).thenReturn(Optional.of(reserva));

            var reservaOptional = reservaJPARepository.findById(id);

            assertThat(reservaOptional).isPresent().containsSame(reserva);

            reservaOptional.ifPresent(reservaArmazenado -> ReservaHelper.validar(reservaArmazenado, reserva));
